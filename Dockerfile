...
RUN mv "$PHP_INI_DIR/php.ini-development" "$PHP_INI_DIR/php.ini" && \
	apk update && \
	apk add --no-cache git openssl-dev openssh-client make nginx sudo mariadb-dev icu-dev bash \
	...
	docker-php-ext-install pdo pdo_mysql imap zip mbstring curl intl && \
	mkdir /run/nginx/ && \
